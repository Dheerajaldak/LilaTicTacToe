...
function updateLeaderboard(player1, player2, result) {
  ...
  const update = (player, res) => {
    leaderboard[player.nickname] = leaderboard[player.nickname] || {
      wins: 0,
      losses: 0,
      draws: 0,
      score: 0,
...
function attemptMatchmaking() {
  while (waitingPlayers.length >= 2) {
    ...
    if (
      io.sockets.sockets.has(player1.id) &&
      io.sockets.sockets.has(player2.id)
    ) {
...
io.on("connection", (socket) => {
  ...
  socket.on("join_matchmaking", (data) => {
    ...
    if (!nickname) return;
    if (
      !playersToGame[socket.id] &&
      !waitingPlayers.some((p) => p.id === socket.id)
    ) {
  ...
  socket.on("make_move", (data) => {
    ...
    if (!player || player.symbol !== game.turn) {
      io.to(socket.id).emit("move_rejected", {
        reason: "Not your turn or invalid player.",
      });
      return;
    }
  ...
  socket.on("cancel_matchmaking", () => {
    ...
    if (waitingIndex !== -1) {
      ...
      io.to(socket.id).emit("matchmaking_status", {
        status: "CANCELED",
        message: "Matchmaking canceled by user.",
      });

      console.log(
        `Client ${socket.id} canceled matchmaking. Removed from queue.`
      );
    } else {
      console.log(
        `Client ${socket.id} tried to cancel but wasn't in queue or game.`
      );
      io.to(socket.id).emit("matchmaking_status", {
        status: "CANCELED",
        message: "Resetting screen state.",
      });
    }
  });
    ...
    if (gameId && activeGames[gameId]) {
      ...
      if (opponent) {
        ...
        io.to(winner.id).emit("opponent_disconnected", {
          message: "Opponent left the game. You win by forfeit (+100pts).",
          leaderboard: getLeaderboard(),
        });
        io.to(loser.id).emit("game_over", {
          board: game.board,
          winner: winner.nickname,
          points: POINTS_LOSS,
          isDraw: false,
          leaderboard: getLeaderboard(),
          myNickname: loser.nickname,
